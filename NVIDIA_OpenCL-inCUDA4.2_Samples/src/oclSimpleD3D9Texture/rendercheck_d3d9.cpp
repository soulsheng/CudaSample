...
{
	char *ref_file_path = shrFindFilePath(ref_file, exec_path);
	...
	std::string fname = ref_file_path;
    FILE *fp = NULL;
	if(NULL == (fp = fopen(fname.c_str(), "rb"))) 
    {
		fname = "../../../src/oclSimpleD3D9Texture/data/";
		fname += ref_file;
		if(NULL == (fp = fopen(fname.c_str(), "rb"))) 
		{
			std::cerr << "Failed to open file: " << ref_file << std::endl;
			return false;
		}
    }
	fclose(fp);

    return (ComparePPM( src_file, fname.c_str(), epsilon, threshold, true ) == true);
};
...
HRESULT CheckRenderD3D9::SurfaceToPPM(IDirect3DDevice9*pDevice, IDirect3DSurface9 *pSurface, const char *zFileName)
{
	...
	SavePPM4ub(zFileName, pPPMData, pDesc.Width, pDesc.Height);
